    ...
    return (
      <div ref={containerRef}>
        ...
        <div className="mt-2.5 flex items-center gap-3 text-xs text-text-muted">
          ...
          <span>donors &middot; {pct.toFixed(1)}% of goal</span>
    ...
    <div ref={containerRef} className="space-y-8">
      ...
      >
        <div className="px-4 py-7 text-center sm:px-7">
          <span ref={raisedRef} className="font-heading text-3xl text-text sm:text-4xl lg:text-5xl">
            ₦0
          </span>
        ...
        <div className="px-4 py-7 text-center sm:px-7">
          <span ref={countRef} className="font-heading text-3xl text-text sm:text-4xl lg:text-5xl">
            0
          </span>
          <p className="mt-2 text-xs font-medium uppercase tracking-widest text-text-muted">
        ...
        <div className="px-4 py-7 text-center sm:px-7">
          <span ref={remainingRef} className="font-heading text-3xl text-text sm:text-4xl lg:text-5xl">
            ₦100M
          </span>
          <p className="mt-2 text-xs font-medium uppercase tracking-widest text-text-muted">
            To Go
      ...
      <div>
        ...
        <div className="relative mb-2 h-5">
          {milestones.map((m) => (
            <span
              key={m.pct}
              className={`absolute -translate-x-1/2 text-xs font-medium transition-colors duration-700 ${
        ...
        <div className="relative">
          <div
            className="h-5 w-full overflow-hidden rounded-full bg-border/40"
            role="progressbar"
            aria-label="Fundraising progress"
            aria-valuenow={Math.round(pct)}
            aria-valuemin={0}
        ...
        <div className="relative mt-2.5 h-3">
          {milestones.map((m) => (
            <div
              key={m.pct}
              aria-hidden
              className={`absolute -translate-x-1/2 h-2 w-2 rounded-full transition-colors duration-700 ${
                pct >= m.pct ? "bg-accent" : "bg-border"
              }`}
              style={{ left: `${m.pct}%` }}
            />
          ))}
