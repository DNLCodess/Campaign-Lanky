...
import { listPollingUnits } from "@/lib/portal/geo";
import { AccountRowActions } from "@/app/portal/_components/account-row-actions";
import { PuAgentForm } from "@/app/portal/ward/accounts/pu-agent-form";

export const dynamic = "force-dynamic";

export default async function WardAccountsPage() {
  const session = await requirePortalRole(["ward_agent"]);
  const [agents, pollingUnits] = await Promise.all([
    listChildAccounts(),
    listPollingUnits(session.lga!, session.ward!),
  ]);
    ...
    <div className="mx-auto max-w-4xl space-y-8">
      <div>
        <h1 className="font-heading text-2xl text-text">
          PU Agents — {session.lga}, Ward {session.ward}
        </h1>
        <p className="mt-1 text-sm text-text-muted">One active agent per polling unit.</p>
      </div>

      <div className="rounded-brand border border-border bg-surface/40 p-5">
        <PuAgentForm pollingUnits={pollingUnits} />
      </div>
          ...
          <tbody>
            ...
            {agents.map((a) => (
              <tr key={a.id} className="border-t border-border align-top">
                ...
                <td className="px-4 py-3 text-text-muted">{a.polling_unit}</td>
